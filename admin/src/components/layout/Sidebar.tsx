      ...
      <ul className="text-[#D9D9D9]">
        {user && (
          <>
            ...
            <hr className="ml-2 mr-2 shadow-md border-[#EFA52F]"/>
            <Link href="/all-appointments hidden sm:block">
              <div className="flex items-center gap-3 py-3.5 px-3 md:px-9 md:min-w-72 cursor-pointer">
                <p className="hidden text-xs md:block">{`${user.nombre.toUpperCase()}`}</p>
              </div>
            </Link>
            <hr className="ml-2 mr-2 text-[#EFA52F] hidden sm:block"/>
